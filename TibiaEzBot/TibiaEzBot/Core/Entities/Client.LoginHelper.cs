...
namespace TibiaEzBot.Core.Entities
{
    public partial class Client
    {
        public class LoginHelper
        {
            ...
            {
                new LoginServer("login01.com"),
                new LoginServer("login02.com"),
                new LoginServer("login03.com"),
                new LoginServer("login04.com"),
                new LoginServer("login05.com"),
            ...
            public bool Login(string login, string password, string charName)
            {
                ...
                {
                    //if (client.GetPlayer().HasFlag(Constants.Flag.InBattle))
                    //{
                    //    if (Report != null)
                ...
                else
                {
                    ...
                    client.Memory.WriteUInt32(Addresses.Client.LoginSelectedChar, 0);
                    if (Report != null)
                        Report.Invoke(State.ResetSelectedCharValue);

                    //reset the char count value..
                    client.Memory.WriteInt32(Addresses.Client.LoginCharListLength, 0);
                    if (Report != null)
            ...
            public void SetCharListServer(byte[] ipAddress, ushort port)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    pointer += 4;
                    client.Memory.WriteString(pointer, ToIPString(ipAddress));
                    pointer += 16;
            ...
            public bool SetCharListServer(CharacterLoginInfo[] charList)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    pointer += 4;
                    client.Memory.WriteString(pointer, ToIPString(BitConverter.GetBytes(charList[i].WorldIP)));
                    pointer += 16;
            ...
            private string ToIPString(byte[] value)
            {
                string ret = "";
                for (int i = 0; i < value.Length; i++)
